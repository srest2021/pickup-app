...
function useQueryGames() {
  ...
  // I don't think we need these fetch by id functions, 
  // since we basically get all games first and set selected game from there.
  // They're kind of a pain to update so I'm commenting them out in case
  // we need them later.
      ...
      if (data) {
        const games = data.map((game: any) => {
          const myGame: MyGame = {
            ...
            joinRequests: game.join_requests
              ? game.join_requests.map((player: any) => {
                  return player as User;
                })
              : [],
            acceptedPlayers: game.accepted_players
              ? game.accepted_players.map((player: any) => {
                  return player as User;
                })
              : [],
          };
