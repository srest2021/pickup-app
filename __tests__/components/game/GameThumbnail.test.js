...
describe("GamesThumbnail", () => {
  test('renders GameThumbnail component without crashing', () => {
      ...
      const { root } = render(<TamaguiProvider config={appConfig}><GameThumbnail navigation={navigation} game={game} /></TamaguiProvider>);
    ...
    test('renders game details correctly', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId("game-description").props.children).toBe("Test description")
    ...
    test('navigates to MyGameView screen on button click', () => {
      ...
      const { root } = render(<GameThumbnail navigation={navigation} game={game} />);
