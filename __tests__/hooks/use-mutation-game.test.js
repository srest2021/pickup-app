...
describe("useMutationGame", () => {
  ...
  it("should edit a game by id", async () => {
    ...
    expect(mockedEditMyGame).toHaveBeenCalledWith(
      expect.any(String),
      expect.any(Object),
    );
  });
  ...
  it("should accept a join request by game and player id", async () => {
    ...
    const mutationGame = useMutationGame();
    await mutationGame.acceptJoinRequestById(
      mockedGameId,
      mockedPlayerId,
      mockedPlusOne,
    );
    ...
    expect(mockedAcceptJoinRequest).toHaveBeenCalledWith(
      mockedGameId,
      mockedPlayerId,
      mockedPlusOne,
    );
  ...
  it("should reject a join request by game and player id", async () => {
    ...
    expect(mockedRejectJoinRequest).toHaveBeenCalledWith(
      mockedGameId,
      mockedPlayerId,
    );
  ...
  it("should remove a player from a game by game and player id", async () => {
    ...
    const mutationGame = useMutationGame();
    await mutationGame.removePlayerById(
      mockedGameId,
      mockedPlayerId,
      mockedPlusOne,
    );
    ...
    expect(mockedRemovePlayer).toHaveBeenCalledWith(
      mockedGameId,
      mockedPlayerId,
      mockedPlusOne,
    );
  ...
  it("should request to join a game by game and player id", async () => {
    ...
    expect(supabase.from().insert).toHaveBeenCalledWith([
      {
        game_id: mockedGameId,
        player_id: mockedPlayerId,
        plus_one: mockedPlusOne,
      },
    ]);
    // Checking if the sendEmailForJoinRequest function was called with the correct arguments
    expect(mutationGame.sendEmailForJoinRequest).toHaveBeenCalledWith(
      "mockedUsername",
      mockedGameTitle,
      mockedOrganizerId,
    );
  ...
  it("should remove a game by its id", async () => {
    ...
    // Checking if the remove_player RPC call was made with the correct arguments
    expect(supabase.from().delete().eq).toHaveBeenCalledWith(
      "id",
      mockedGameId,
    );
  ...
  it("should send an email notification to friends", async () => {
    ...
    const mutationGame = useMutationGame();
    await mutationGame.sendEmailToFriends(
      mockedUsername,
      mockedTitle,
      mockedDatetime,
    );
