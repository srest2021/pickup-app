...
describe('useMutationSports', () => {
  ...
  beforeEach(() => {
    ...
    useStore.mockReturnValueOnce([session, userSports, setLoading, addUserSport, editUserSport]);
  });
  ...
  describe('setSport', () => {
    it('should add a new sport if it does not exist in userSports', async () => {
      ...
      const result = await mutationSports.setSport(mockedSportName, mockedSportSkillLevel);

      // Check if addSport was called with the correct arguments
      expect(mockAddSport).toHaveBeenCalledWith(mockedSportName, mockedSportSkillLevel);
    ...
    it('should display an alert when there is no user session', async () => {
      ...
      useStore.mockReturnValueOnce([null, userSports, setLoading, addUserSport, editUserSport]);
      ...
      expect(setLoading).toHaveBeenNthCalledWith(2, false);
       ...
       describe('addSport', () => {
         it('should add a new sport', async () => {
           ...
           mockSupabase.from().insert.mockResolvedValueOnce({ data: [{ id: 'mockedSportId' }] });
     
           // Initialize the hook
           const mutationSports = useMutationSports();
     
           // Call the addSport function
           const result = await mutationSports.addSport(mockedSportName, mockedSportSkillLevel);
       ...
       describe('editSport', () => {
         it('should edit an existing sport', async () => {
           const mockedSport = { id: 'mockedSportId', name: 'Basketball', skillLevel: SkillLevel.Beginner };
         ...
         it('should display an alert when there is no user session', async () => {
           ...
           // Call the editSport function
           await mutationSports.editSport({ id: 'mockedSportId', name: 'Basketball', skillLevel: SkillLevel.Beginner });
